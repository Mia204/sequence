...
num1 <- abs(-78.2)
len1 <- str_length(str2)
str3 <- toString(num1)
num2 <- as.numeric(str3)
```
...
val5 <- vec1[5]
vec1_crop = vec1[-20]
vec1_subset = vec1[c(10,15)]
vec1_lower = vec1[vec1 < 6]
```
...
```{r}
################## ADD YOUR CODE UNDER THIS LINE ###
matrix3 = matrix(c(3,12,21,6,15,24,9,18,27), nrow = 3, ncol = 3)
matnum1= matrix3[2,3]
col3 = matrix3[,3]
```
...
```{r}
################## ADD YOUR CODE UNDER THIS LINE ###
mylist <- list("the","quick","brown","fox","jumps","over","the","lazy","dog")
onewordlist <- mylist[5]
ele_seven <- mylist[[7]]
even_vec <- c(mylist[[2]],mylist[[4]],mylist[[6]],mylist[[8]])
#or how about this one, illustrating how there is always more than one way to do things!
even_vec<-unlist(mylist[c(2,4,6,8)])
```
...
```{r}
################## ADD YOUR CODE UNDER THIS LINE ###
pop_contest <- data.frame(language = c("Linux","Python","R"), popularity = c(25,35,40))
#View(pop_contest)
Pyscore <- pop_contest[2,2]
#by name
scores <- pop_contest$popularity
#by number
scores <- pop_contest[,2]
highscores <- pop_contest[pop_contest$popularity == 35,]
```
